...
첫 화면은 기존 세 프레임을 어긋난 계단 형태로 배치하고, 작은 검은 td가 바닥·아래층·중간층·위층을 오르내리는 구성입니다. d 상단의 노란 점을 브랜드 포인트로 사용합니다. 위층에서 잠깐 둘러보고 반대편 바닥으로 내려온 뒤 쉬며, 다음 경로는 반대 방향으로 움직입니다. 오브젝트를 누르면 다시 움직입니다. 연속 클릭은 착지가 끝난 뒤 처리합니다. 세 개의 모드 버튼으로 프레임 간격과 방향을 바꾸고, 멈추기 버튼으로 모션을 정지할 수 있습니다.
