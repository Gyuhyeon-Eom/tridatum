...
첫 화면은 기존 세 프레임을 어긋난 계단 형태로 배치하고, 작은 검은 td가 바닥·아래층·중간층·위층을 오르내리는 구성입니다. d 상단의 노란 점을 브랜드 포인트로 사용합니다. 별도 시작·정지 버튼 없이 자동으로 움직이며, 착지할 때마다 인접 층 또는 같은 층의 다른 위치를 무작위로 선택합니다. 이동 위치와 점프 높이·속도가 달라지고, 긴 대기 없이 다음 점프로 이어집니다. 세 개의 모드 버튼으로 프레임 간격과 방향을 바꿀 수 있습니다.
